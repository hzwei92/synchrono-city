          ...
          (let [child (.child doc i)]
            ...
            (let [node-type (.-name (.-type child))]
              (case node-type
                "paragraph"
                (do
                  ;; Add newline before paragraph if not first paragraph
                  (when (pos? i)
                    (swap! fragments conj "\n"))
                    ...
                    (let [inner-node (.child child j)
                      ...
                      (case inner-type
                        ...
                        "event" (let [event-id (.. inner-node -attrs -eventId)
                                    preposition (.. inner-node -attrs -preposition)]
                                  (log/debug "Found event node:"
                                           {:id event-id
                                            :preposition preposition})
                                  (when event-id
                                    (swap! tags conj ["e" event-id (when preposition preposition)])
                                    (swap! fragments conj (str "[e " event-id (when preposition (str " " preposition))"]"))))

                        "pubkey" (let [pubkey (.. inner-node -attrs -pubkey)]
                                   (swap! tags conj ["p" pubkey])
                                   (swap! fragments conj (str "[p " pubkey "]")))
                        "relay" (let [relay (.. inner-node -attrs -relay)]
                                  (swap! tags conj ["r" relay])
                                  (swap! fragments conj (str "[r " relay "]")))

                        "timestamp" (let [timestamp (.. inner-node -attrs -timestamp)]
                                      (swap! tags conj ["t" timestamp])
                                      (swap! fragments conj (str "[t " timestamp "]")))

                        "geohash" (let [geohash (.. inner-node -attrs -geohash)]
                                    (swap! tags conj geohash)
                                    (swap! fragments conj geohash)))

                      nil)))

                  ;; Remove explicit newline addition between paragraphs since we handle it at start of paragraph
                  ))
   ...
   (fn [{:keys [db]} _]
     ...
     (let [editor-view (:editor/view db)
       ...
       (cond
         ...
         (not private-key)
         (do
           (log/error "Cannot publish without private key")
           {:db db
            :dispatch [:notifications/add
                       {:type :error
                        :message "Cannot publish without private key. Please add your private key in settings."}]})
